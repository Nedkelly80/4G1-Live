...
def engine_state(ctx):
    """Derive engine_running/warm/stopped/idle_like from a live ctx dict.

    Shared by sensor_health() and the baseline layer so both bucket readings
    against the exact same notion of "what the car was doing" at the time.
    """
...
def sensor_health(pid, value, ctx=None, history=None):
    ...
    if cool is None:
        cool = _ctx_get(ctx, 0x10)
    es = engine_state(ctx)
    engine_running = es["engine_running"]
    warm = es["warm"]
    stopped = es["stopped"]
    idle_like = es["idle_like"]
